...
- Markdown 편집 모드 및 저장
- 새 Markdown 문서 생성, 이름 변경, 삭제
- 저장 전 dirty 상태 표시
- `Cmd/Ctrl+S` 단축키 저장
- 저장·생성·이름 변경·삭제 후 inline 성공 피드백
- 외부 파일 변경 감지 및 dirty draft 충돌 보호
- 문서 이동, 새로고침, 파일 액션, 창 닫기 전 unsaved-change 보호 dialog
...
- 저장, 새 문서 생성, 이름 변경, 삭제가 성공하면 화면 안에 짧은 성공 메시지를 표시합니다.
- 다른 문서로 이동, 새 문서 생성, 이름 변경, 삭제, 새로고침, 창 닫기처럼 현재 draft를 잃을 수 있는 동작은 먼저 확인 dialog를 표시합니다.
  - **저장 후 계속**: 현재 draft를 저장한 뒤 요청한 동작을 이어서 실행합니다.
  - **변경사항 버리고 계속**: 현재 draft를 버리고 요청한 동작을 실행합니다.
  - **취소**: 요청한 동작을 중단하고 편집 상태를 유지합니다.
